...
describe('[UserProfileLocationService]', () => {
    ...
    it('When creating a profile, throws exception because of unexistent city or user', async () => {
        ...
        const userProfileLocationService = new UserProfileLocationService(
            locationServiceMock, profileServiceMock, userServiceMock, dataSourceMock
        );

        try{
            const userProfileLocation = await userProfileLocationService.createUserProfile({
                username: 'pepe', password: 'my-pass', name: 'Pepe Gonzales', address: 'Bakery St', cityId: 15
            });
        } catch (e) {
    ...
    it('Gets a profile using an userId', async () => {
        ...
        getFullAddressMock.mockResolvedValue({street: 'Bakery St', cityName: 'Marsella', countryName: 'France'} as FullAddressDto);

        const userProfileLocationService = new UserProfileLocationService(
            locationServiceMock, profileServiceMock, userServiceMock, dataSourceMock
        );

        const profileLocationAddress = await userProfileLocationService.getUserProfileAndLocation('pepe');
